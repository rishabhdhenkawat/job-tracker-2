...
def main_db():
    ...
    response = app.response_class(
        response=dumps(jl.fetch_main(date = date, hours = hours, lsearch = lsearch, csearch = csearch, tsearch = tsearch, limit = limit, page = page, portal = portal,dept=dept), indent=2),
        status=200,
...
def get():
    ...
    result=jl.get_file(all=True)
    import pandas as pd
    df = result
    #print(df,flush=True)
    df.to_csv('static/client/csv/jobs.csv')
    return  send_from_directory(app.config["CLIENT_CSV"], filename='jobs.csv', as_attachment=True)
    #response = app.response_class(response=dumps(df),status=200,mimetype='application/json')
    #return response


@app.route('/db/get_all', methods=['GET'])
def get_all():
    field = request.args.get('field')
    response = app.response_class(
        response=dumps(jl.get_all(field), indent=2),
        status=200,
        mimetype='application/json'
    )
    return response
